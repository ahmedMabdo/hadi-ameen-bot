#!/usr/bin/env python3
"""Hadi Ameen - Mars Team daily digest routine (Discord REST API v10 direct).

Commands:
  python3 discord_marsteam.py fetch -> prints JSON of messages from the
      last rolling 24 hours in the mars-team channel to stdout
  python3 discord_marsteam.py post "<text>" -> posts text to the mars-team channel
  python3 discord_marsteam.py post --dry-run "<text>" -> dry-run mode: does not
      post publicly

Dry-run mode is enabled via env MARSTEAM_DRY_RUN=true/1/yes, or the --dry-run
flag. In dry-run mode the summary is only printed to stdout, and optionally
DMed to MARSTEAM_DM_USER_ID if set - nothing is posted publicly.

Every error is printed as "MARSTEAM: ..." to stderr and returns a non-zero
...
def resolve_channel_id() -> str | None:
    """Returns the mars-team channel id (env override MARSTEAM_CHANNEL_ID, else the fixed default)."""
    return os.environ.get("MARSTEAM_CHANNEL_ID", MARS_TEAM_CHANNEL_ID)


def fetch_last_24h_messages() -> list[dict]:
    """Fetch messages from the mars-team channel in a rolling 24h window ending now (Cairo time)."""
...
def log(msg: str) -> None:
    print(f"MARSTEAM: {msg}", file=sys.stderr)
...
def is_dry_run(cli_flag: bool = False) -> bool:
    return cli_flag or is_truthy(os.environ.get("MARSTEAM_DRY_RUN"))
...
def post_message(content: str, dry_run: bool = False) -> bool:
    if dry_run:
        print("MARSTEAM: DRY RUN — لم يُنشر في القناة العامة")
        print(content)

        dm_user_id = os.environ.get("MARSTEAM_DM_USER_ID")
        if dm_user_id:
            ...
            log("MARSTEAM_DM_USER_ID مش محدد — الملخص اتطبع في stdout بس من غير DM")
...
def main() -> None:
    ...
    if not args:
        print("Usage: discord_marsteam.py [--dry-run] fetch | post \"<text>\"", file=sys.stderr)
        sys.exit(1)

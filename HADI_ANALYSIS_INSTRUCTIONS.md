...
> ملحوظة أدوات: `PostHog` متصل فعلًا على كل مشاريع 8Orders، والبيانات بتيجي منه (عبر `posthog_cli.py` أو روتين `intel` اليومي) أو من اللي متبعت في القناة. أحيانًا مايرجّعش بيانات بسبب حد الاستخدام المجاني (free tier) — ساعتها قول «البيانات مؤقتًا مش متاحة بسبب حد الـ free tier» من غير ما تنكر إن الأداة موجودة أو تدّعي إنها رجّعت نتيجة. بيانات التذاكر من `ado_cli.py`.
